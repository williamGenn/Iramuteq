...
if __name__ == '__main__':
    #on importe les librairies manquantes
    try:
        import os
        import math
...
# title, article_date, lang, score,selection_status
    #on crée la fenetre tkinter    
    root2 = Tk()
    root2.withdraw()
    root2.title('test')
    root2.iconbitmap("c:")
    #on demande a l'utilisateur de selectionner un fichier CSV
    root2.filename= filedialog.askopenfilename( title="select A file",filetypes= (("CSV file","*.csv"),("All files","*.*")))  
    
    
    #creation de la dataframe
    df = pandas.read_csv(root2.filename,sep = ';', error_bad_lines=True, index_col = 0 ,skip_blank_lines= True)
    df.fillna('')
    root2.destroy()    
    #affichage des resuatsts dans la fenetre tkinter via un affichage type excel
    mc = Multicolumn_Listbox(root, ["title", "article_date", "lang", "score", "selection_status"], stripped_rows = ("white","#f2f2f2"), command=on_select, cell_anchor="center")
    mc.interior.pack()
    compteur = df.shape[0]
    i=0
    y=0
    ...
    print("Lancement ...")
    numchunktoread = 10000 # lecture par bloc pour optimiser
    hexdata = bytearray(numchunktoread) # les blocs lus sont des "bytearray" de numchunktoread size (contenu integer de 0 à 256, à utiliser sous forme b'\xXX')

    lengthfichier = os.path.getsize(root2.filename) #taille fichier source

    with open(root2.filename, "rb") as csv_file , open('puversion.csv', mode='wb') as employee_file: #lecture en binaire , ecriture en binaire
 
        read_count = 0
        last_block = 0
        remainingtoread = lengthfichier #taille initiale à lire
        while True and last_block == 0:
            if (remainingtoread<numchunktoread) :
                numchunktoread = remainingtoread #si le reste à lire est plus petit qu'un bloc, il devient le reste à lire
                last_block = 1                   #on a identifié le dernier blov à lire
            else:
                remainingtoread = remainingtoread - numchunktoread  #sinon on ajuste le "remaining to read" en retirant le bloc
               
            hexdata = csv_file.read(numchunktoread)                 # lecture du bloc de la taille
           
         
            # filtrage caractères bloquant IRaMuTeQ / on utilise .replace comme pour des strings
            hexdata = hexdata.replace(b'\x9d', b' ') #identifié à la main après tentative IRaMuTeQ
            hexdata = hexdata.replace(b'\x90', b' ') #identifié à la main après tentative IRaMuTeQ
            hexdata = hexdata.replace(b'\x81', b' ') #identifié à la main après tentative IRaMuTeQ
            hexdata = hexdata.replace(b'\x8d', b' ') #identifié à la main après tentative IRaMuTeQ
            hexdata = hexdata.replace(b'\x8f', b' ') #identifié à la main après tentative IRaMuTeQ
            hexdata = hexdata.replace(b'\x0d\x0a\x09', b'   ')           #saut de ligne "title" (CR+ Tab) - remplace par des spaces
            hexdata = hexdata.replace(b'\x0d\x0a\x4d', b'\x20\x20\x4d')  #saut de ligne "title" ( CR+"I") - on ne change que le CR
            hexdata = hexdata.replace(b'\x0d\x0a\x49', b'\x20\x20\x49')  #saut de ligne "title" (CR+ "M"- on ne change que le CR
            employee_file.write(hexdata) # ecriture du bloc filtré
           
            # on ajuste le reste à lire
            read_count = read_count + numchunktoread
   
    # pour info et s'assurer que tout est lu et transformé
    print ("longueur du fichier:"+str(lengthfichier))
    print ("contenu verifié:"+str(read_count))
    print("terminé")
    
    #affichage des valeurs de la dataframe
    while i != compteur:
        
        str1=df['title'].values[i]
        str2=df['article_date'].values[i]
        str3=df['lang'].values[i]
        str4=df['score'].values[i]
        str5=df['selection_status'].values[i]
        #on affiche pas les NaN dans la fenetre
        if math.isnan(df['selection_status'].values[i]) == True:
            str5 = " "
        mc.insert_row([str1,str2,str3,str4,str5])
        i=i+1
